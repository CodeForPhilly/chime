...
hosp_capacity = st.sidebar.number_input(
    "Hospital Capacity",  value=1000, step=100, format="%i"
)

icu_capacity = st.sidebar.number_input(
    "ICU Capacity",  value=300, step=50, format="%i"
)

vent_capacity = st.sidebar.number_input(
    "Ventilator Capacity",  value=200, step=50, format="%i"
)
...
st.markdown(
    "Projected number of hospital resources."
)
...
for k, word in wording_dict.items():
    try:
        cap_day = census_df[census_df[k] < 0]['day'].iloc[0]
        st.markdown(
            "* {} will reach capacity on day **{}**.".format(word, cap_day)
        )
    except Exception as e:
        print(e)

if st.checkbox("Show Projected Hospital Resources in tabular form"):
    st.dataframe(census_table)
