...
intrinsic_growth_rate = 2 ** (1 / doubling_time) - 1 if doubling_time > 0 else 0
...
census_df = build_census_df(projection_admits, hosp_los, icu_los, vent_los)

# PRESENTATION
st.subheader("New Admissions")
st.markdown("Projected number of **daily** COVID-19 admissions at Penn hospitals")
st.altair_chart(
    new_admissions_chart(alt, projection_admits, n_days - 10, as_date=as_date),
...
st.altair_chart(
    admitted_patients_chart(alt, census_df, n_days - 10, as_date=as_date),
    use_container_width=True
)
...
if st.checkbox("Show Additional Projections"):
    show_additional_projections(st, alt, additional_projections_chart, i, r, as_date=as_date)
    if st.checkbox("Show Raw SIR Simulation Data"):
        draw_raw_sir_simulation_table(st, n_days, s, i, r, as_date=as_date)
